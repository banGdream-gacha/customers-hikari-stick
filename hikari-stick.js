...
const theRandom = id => {
    ...
    if (!(id in randomStorage)) randomStorage[id] = Math.random();
    return randomStorage[id];
}
...
function createCustomers(rank) {
    ...
    for (let i = 0; i < ln; i++) {
        ...
        const sn = Number(Math.ceil(main.offsetWidth / 80)) + Number(Math.floor(theRandom(`i${i}sn`) * -2)) + 3 + (rank == 5 ? 5 : 0);
        for (let j = 0; j < sn; j++) {
            ...
            let color = (rank => {
                switch (rank) {
                    ...
                    case 4:
                        ...
                        return [star4Color[Math.floor(theRandom((changeColor ? `i${i}j${j}s4c` : null)) * star4Color.length)]];
                    case 5:
                        let star5Color = ["white", "yellow", "red", "green", "blue", "plum"];
                        return [star5Color[Math.floor(theRandom((changeColor ? `i${i}j${j}s5c` : null)) * star5Color.length)], "star5"];
                    default:
                        throw "Error!";
                }
            })(rank);
            stick.querySelector(".stick-body").classList.add(...color);
            stick.querySelector(".stick-body").style.left = String(Number(Math.floor(theRandom(`i${i}j${j}lp`) * -21)) + 60) + "px";
